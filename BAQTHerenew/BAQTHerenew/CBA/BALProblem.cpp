...
BALProblem::BALProblem(const std::string &filename, bool use_quaternions) { //Here I never use it 
	return;
...
void BALProblem::WriteToFile(const std::string &filename) const {
	...
	fprintf(fptr, "%d %d %d\n", num_observations_, num_poses_, num_points_);
	...
	*/
    for (int i = 0; i < num_poses(); ++i) {
		...
		*/ //Adding 22/10/2019 , it is so strange that use_quaternions_ is true, I never use it !!!
		memcpy(angleaxis, poses() + 6 * i, 6 * sizeof(double));
        for (double item : angleaxis)
            fprintf(fptr, "%g\n", item);
    }
    for (int i = 0; i < num_points(); ++i) {
        const double *point = points() + i * point_block_size(); //这里明显出问题了，比如在i == 28的时候，输出了异常的值。这里其实是对points()这个地址，我们让其不断的跨越，从而实现对points的定位，这一步很重要
        // const double *points() const { return poses() + pose_block_size() * num_poses_; }//把姿态过了之后，就是我们的points了，这就是参数的奥妙
		//const double *poses() const { return parameters_ + camera_block_size() * num_cameras_; }//把相机参数过了之后，就是六个姿态了
...
void BALProblem::PoseToAxisAndCenter(const double *pose,
    ...
    else */ //Modified 22/10/2019 wired , I don't use qua
...
void BALProblem::Perturb(const double rotation_sigma,
    ...
    for (int i = 0; i < num_poses_; ++i) {
        ...
        if (translation_sigma > 0.0)
            PerturbPoint3(translation_sigma, pose + pose_block_size() - 3); //为啥要减31029，给平移加噪声
    }
}
/*
void BALProblem::setParaNums(int num_observations, int num_cameras, int num_poses, int num_points) {
    ...
    point_index_ = new int[num_observations_];
    observations_ = new double[2 * num_observations_];
    num_parameters_ = 9 * num_cameras_ + 6 * num_poses_ + 3 * num_points_; //here is all our parmeters 
    parameters_ = new double[num_parameters_]; // and we define it here in this way 
}
*/
void BALProblem::setParaNums(int num_observations, int num_poses, int num_points) {
	num_observations_ = num_observations;
	//num_cameras_ = num_cameras;
	num_poses_ = num_poses;
	num_points_ = num_points;
	...
	point_index_ = new int[num_observations_];
	observations_ = new double[2 * num_observations_];
	num_parameters_ = 6 * num_poses_ + 3 * num_points_; //here is all our parmeters 
	parameters_ = new double[num_parameters_]; // and we define it here in this way 
}
